...
export function OptionModal({ item, isOpen, onClose, onConfirm }: OptionModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
                ...
                <div className="relative shrink-0">
                    ...
                    <button
                        onClick={onClose}
                        className="absolute top-3 right-3 z-10 w-9 h-9 flex items-center justify-center bg-white/95 backdrop-blur-sm rounded-full shadow-lg text-[var(--color-coffee-700)] hover:bg-white hover:scale-110 transition-all"
                    >
                    ...
                    {/* Hero Image */}
                    {item.image ? (
                        <div className="relative h-56 bg-gradient-to-b from-[var(--color-coffee-100)] to-[var(--color-coffee-50)] overflow-hidden">
                            <img
                                src={item.image}
                                alt={item.name}
                        ...
                        <div className="h-40 bg-gradient-to-br from-[var(--color-primary)]/10 via-[var(--color-coffee-50)] to-[var(--color-coffee-100)] flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 text-[var(--color-coffee-300)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="bg-white px-5 py-4 shadow-sm">
                        <h3 className="font-bold text-2xl text-[var(--color-coffee-900)] mb-2 leading-tight">{item.name}</h3>
                        <div className="flex items-center justify-between">
                            <span className="text-3xl font-bold text-[var(--color-primary)]">฿{item.price}</span>
                            {item.description && (
                                <p className="text-sm text-[var(--color-coffee-500)] max-w-[60%] line-clamp-1">{item.description}</p>
                            )}
                        </div>
                    </div>
                </div>
