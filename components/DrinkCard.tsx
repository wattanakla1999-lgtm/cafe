...
export function DrinkCard({ item, onClick, showRecommendedBadge, showBestSellerBadge, bestSellerRank }: DrinkCardProps) {
    return (
        <div
            onClick={onClick}
        ...
        >
            ...
            <div className="aspect-[4/3] w-full overflow-hidden rounded-t-xl bg-[var(--color-coffee-50)] relative">
                ...
                {showBestSellerBadge && bestSellerRank && (
                    <div className={`absolute top-2 left-2 ${bestSellerRank === 1 ? 'bg-gradient-to-r from-red-500 to-pink-600' :
                        ...
                        } text-white text-xs font-bold px-2 py-1 rounded-full shadow-md flex items-center gap-1 z-10`}>
                        {bestSellerRank <= 3 ? (
                ...
                {showRecommendedBadge && (
                    <div className="absolute top-2 right-2 bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-md flex items-center gap-1 z-10">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>
                        <span>แนะนำ</span>
