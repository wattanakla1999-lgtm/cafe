...
export function SummaryCards({ summary }: SummaryCardsProps) {
    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {cards.map((card, idx) => (
                <div key={idx} className="bg-white p-3 md:p-4 rounded-xl shadow-sm border border-[var(--color-coffee-100)] flex flex-col md:flex-row items-center md:items-center gap-2 md:gap-4 text-center md:text-left">
                    <div className={`p-2 md:p-3 rounded-full ${card.bg} shrink-0`}>
                        {card.icon}
                    </div>
                    <div className="min-w-0 w-full">
                        <p className="text-[10px] md:text-xs text-[var(--color-coffee-500)] uppercase font-bold truncate">{card.label}</p>
                        <p className="text-lg md:text-xl font-bold text-[var(--color-coffee-900)] truncate">{card.value}</p>
                    </div>
                </div>
            ))}
        </div>
