...
export default function CounterPage() {
    const { menuItems, categories: contextCategories, discounts, hasMore, loadMoreMenuItems, refetchMenu, isLoading, isFetchingMore, initializeMenu } = useMenu();
    ...
    return (
        <ProtectedRoute>
            {/* Full-page loading indicator during initial load */}
            {isLoading && menuItems.length === 0 ? (
                <div className="min-h-screen bg-[var(--color-bg)] flex items-center justify-center">
                    <div className="flex flex-col items-center gap-4">
                        <div className="w-12 h-12 border-4 border-[var(--color-coffee-200)] border-t-[var(--color-primary)] rounded-full animate-spin"></div>
                        <p className="text-[var(--color-coffee-500)] font-medium">กำลังโหลด...</p>
                    </div>
                </div>
            ) : (
                <div className="min-h-screen bg-[var(--color-bg)] flex flex-col lg:flex-row h-screen overflow-hidden relative">
                        ...
                        <div className="bg-white border-b border-[var(--color-coffee-100)] shrink-0">
                            ...
                            <div className="px-4 pb-2">
                                <div className="relative" ref={searchRef}>
                                    ...
                                    <input
                                        type="text"
                                        className="block w-full pl-9 pr-3 py-2 border border-[var(--color-coffee-200)] rounded-lg leading-5 bg-[var(--color-coffee-50)] text-[var(--color-coffee-900)] placeholder-[var(--color-coffee-400)] focus:outline-none focus:bg-white focus:placeholder-[var(--color-coffee-300)] focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] sm:text-sm transition-all"
                                    ...
                                    {showSuggestions && suggestions.length > 0 && (
                                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                            {suggestions.map((suggestion, index) => (
                                                <div
                                                    key={index}
                                                    className="px-4 py-2 text-sm text-[var(--color-coffee-700)] hover:bg-[var(--color-coffee-50)] cursor-pointer flex items-center gap-2"
                                                    onClick={() => {
                    ...
                    <div className={`w-full lg:w-80 xl:w-96 bg-white border-l border-r border-[var(--color-coffee-200)] flex-col h-full shrink-0 shadow-lg z-20 ${activeTab === "cart" ? "flex" : "hidden lg:flex"} `}>
                        ...
                        <div className="p-2 lg:p-4 bg-white border-t border-[var(--color-coffee-200)] space-y-1 lg:space-y-4 shrink-0 mb-24 lg:mb-0">
                            ...
                            <div ref={submitButtonRef} data-tour="confirm-order-button">
                                <Button
                                    fullWidth
                                    size="lg"
                    ...
                    <div ref={queueTargetRef} className={`h-full shrink-0 z-30 transition-all duration-300 ${activeTab === "queue" ? "flex w-full" : "hidden lg:flex"} `} data-tour="queue-button">
                    ...
                    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] border-t border-[var(--color-coffee-200)] flex justify-around p-3 z-[100]">
